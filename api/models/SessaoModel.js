...
export const criar = async (usuario, validade) => {
  ...
  try {
    ...
    const cmdSql = `INSERT INTO sessao (usuario, token, validade) VALUES (?, ?, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL ? HOUR))`;
    const [result] = await cx.query(cmdSql, [usuario, token, validade]);
    ...
    return await consultarPorId(lastIdUser, cx);
...
/**
 * Consulta uma sessão ativa no banco de dados a partir do ID da sessão.
 *
 ...
 *
 * @param {number} id - ID único da sessão.
 ...
 */
const consultarPorId = async (id, cx = null) => {
  let localCx = cx;
  try {
    if (!localCx) {
      localCx = await pool.getConnection();
    }
    const cmdSql = "SELECT * FROM sessao WHERE id = ?;";
    const [rows] = await localCx.query(cmdSql, [id]);
 ...
 *
 * @param {number} sessao_id - ID único da sessão.
 ...
 */
export const buscarSessao = async (sessao_id, usuario_id, token) => {
  try {
    const sessao = await consultarSessoaDoUsuario(sessao_id, usuario_id);
 ...
 */
export const extender = async (id, tempo_em_horas) => {
  let cx;
  try {
    const cmdSql =
      "UPDATE sessao SET validade = DATE_ADD(validade, INTERVAL ? HOUR) WHERE id = ?;";
    cx = await pool.getConnection();
    const [rows] = await cx.query(cmdSql, [tempo_em_horas, id]);
...
/**
 * Consulta uma sessão ativa no banco de dados a partir do ID da Sessão e do ID do usuário.
 *
 * - Utilizada para verificar se existe uma sessão vinculada a um determinado usuário.
 * - Pode receber uma conexão externa (útil em transações) ou criar a sua própria.
 * - Retorna todos os campos da sessão encontrada (id, usuario, token, validade, etc).
 ...
 * @throws {Error} Caso ocorra falha na execução da query ou no acesso ao banco de dados.
 */
const consultarSessoaDoUsuario = async (sessao_id, usuario_id, cx = null) => {
  let localCx = cx;
  try {
    if (!localCx) {
      localCx = await pool.getConnection();
    }
    const cmdSql = "SELECT * FROM sessao WHERE id = ? and usuario = ?;";
    const values = [sessao_id, usuario_id];
    const [rows] = await localCx.query(cmdSql, [values]);
    return (rows.length > 0) ? rows[0] : null;
  } catch (error) {
    throw new Error("Erro ao buscar usuário por ID: " + error.message);
  } finally {
